...
def create_lookup_tables(text):
    ...
    int_to_vocab = {c : i for c, i in enumerate(vocab)}
...
def token_lookup():
    ...
    # TODO: Implement Function
    symbols = ['.', ',', '"', ';', '!', '?', '(', ')', '--', '\n']
    tokens = ['#Period', '#Comma', '#Quotation_Mark', '#Semicolon', '#Exclamation_mark',
...
def pick_word(probabilities, int_to_vocab):
    ...
    idx_arr = np.where(probabilities == max_prob)
    idx = idx_arr[0][0]
    pred_wd = int_to_vocab[idx]
    return pred_wd
